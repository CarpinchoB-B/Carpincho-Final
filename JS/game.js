...
const windowClicks = document.getElementById('clicks');
const damageInfo = document.getElementById('click_dmg');
...
if(localStorage.getItem('currentSave') == 'First' || localStorage.getItem('currentSave') == 'Second' || localStorage.getItem('currentSave') == 'Third')
{
    if(localStorage.getItem('NewGame') != 'true')
    {
        ...
        increasedMoney = Number(localStorage.getItem('increase'));
...
damageInfo.textContent = damage;

if(localStorage.getItem('clicks') != null)
{
    windowClicks.textContent = localStorage.getItem('clicks');
}
...
save.addEventListener('click',()=>
{
    ...
    localStorage.setItem('increase',increasedMoney);
    localStorage.setItem('difficult',localStorage.getItem('difficult'));
    localStorage.setItem('damage',damage);
    localStorage.setItem('clicks',clicks);
    localStorage.setItem('time',timeIn);
    localStorage.setItem('increaseTime',timeIncrease);
    localStorage.setItem('timeThis',timeThis);
    localStorage.setItem('isAbility',isAbility);
...
function IncreaseMoney()
{
    ...
    timeIn++;
    timeInStatistics.textContent = timeIn + " seconds";
}
    ...
    if(Number(testDamage.value) > 0)
    {
        ...
        if(hp - damage <= 0 && enemyNumber <= 9)
        {
            ...
            if(enemyNumber > 9)
            {
                console.log("You win");
                win.play();
                testDamage.value = 'YOU WIN';
                enemyForeground.src = "enemy/win.png";
            }
        }
        else
        {
...
function UseAbility(time)
{
    ...
    if(time === 0)
    {
        ...
        interval = setInterval(() => {
            time++;
            coolDownFirst.textContent = 10 - time;
            console.log(time);
            if(time === 10)
            {
                time = 0;
...
{
    clicks++;
    windowClicks.textContent = clicks;
    DealDamage(damage);
});
...
document.addEventListener("keydown",function(event)
{
    ...
    else if(event.key === "f" && isAbility == true || event.key === "F" && isAbility == true)
    {
        ...
        {
            timeThis++;
            coolDownSecond.textContent = 10 - timeThis;
            if(timeThis === 10)
            {
                timeThis = 0;
